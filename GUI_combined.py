...
class ChooseStock:
    ...
    def find_symbol(self):
        ...
        key_q = tk.Label(text="Please enter Alphavantage key: ", font = self.text_font)
        self.key = tk.Entry(font = self.text_font)
        key_q.pack(pady=5, padx=5)
        self.key.pack(pady=5, padx=5)
             
        keywords_q = tk.Label(text="Please enter company name: ", font = self.text_font)
        self.keywords = tk.Entry(font = self.text_font)
        keywords_q.pack(pady=5, padx=5)
        self.keywords.pack(pady=5, padx=5)
        
        find = tk.Button(text="Search", width = 10, font = self.button_font, command = lambda: self.symb_names(self.keywords.get(), self.key.get()))
        find.pack(pady=10, padx=10)
        exit = tk.Button(text="Quit", width = 10, font = self.button_font, command = self.quit)
    ...
    def enter_symbol(self):
        ...
        stock_q = tk.Label(text="Please enter company symbol: ", font = self.text_font)
        self.stock = tk.Entry(font = self.text_font)
        stock_q.pack(pady=5, padx=5)
        self.stock.pack(pady=5, padx=5)
        
        start_date_q = tk.Label(text = "Please enter start date in YYYY-MM-DD format: ", font = self.text_font)
        self.start_date = tk.Entry(font = self.text_font)
        start_date_q.pack(pady=5, padx=5)
        self.start_date.pack(pady=5, padx=5)
        
        enddate_q = tk.Label(text = "Please enter end date in YYYY-MM-DD format: ", font = self.text_font)
        self.end_date = tk.Entry(font = self.text_font)
        enddate_q.pack()
        self.end_date.pack(pady=10, padx=10)

        enter = tk.Button(text="Get Data", width = 10, font = self.button_font, command = lambda: self.req_to_frame(self.key.get(), self.stock.get(), self.start_date.get(), self.end_date.get())) 
