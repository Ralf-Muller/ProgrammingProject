...
def val_window(win_size):
    try:
        ...
        raise tk.messagebox.showerror("Error",
            "Please enter a number for your Window Size")
    return win_size
...
def val_std(st_dev):
    try:
        ...
        raise tk.messagebox.showerror("Error", 
            "Please enter a number for your Standard Deviation")
    return num_std
...
def get_ema(emas):
    try:
        ...
        raise tk.messagebox.showerror("Error", 
           "Please enter a number for your EMA and Signal Line")
    return ema
...
def val_data_macd(fema, sema, smooth):
    ...
    if sema >= fema:
        raise tk.messagebox.showerror("Error",
           'Please enter a Fast EMA greater than the Slow EMA')
...
def plot_trend_line(stocks, company, t_size, size = 1):
    stkols.val_company(stocks, company)    
    inputs, targets, x_train, x_test, y_train, y_test = stkols.gen_model(
...
def plot_bollinger(stocks, company, win_size, stdev):
    ...
    plt.xlabel('Date', fontsize = 10)
    plt.ylabel('Adj. Closing Price', fontsize = 10)
    plt.plot(month, stocks[company[0]]['5. adjusted close'], 
...
def plot_wma(stocks, company, win_size):
    ...
    plt.xlabel('Date', fontsize = 10)
    plt.ylabel('Adj. Closing Price', fontsize = 10)
    wma = stocks[company[0]]['5. adjusted close'].rolling(
            window =win_size).apply(lambda x: np.sum(weights*x))
    plt.plot(month, price, label="Closing Prices")
...
# The code for MACD adapted for the project was obtained from 
...
def obtain_macd(stocks, company, fema, sema, smooth):
    ...
    macd = pd.DataFrame(fast - slow).rename(columns = {'5. adjusted close':
        'macd'})
    signal = pd.DataFrame(macd.ewm(span = smooth, 
                   adjust = False).mean()).rename(columns = {'macd':'signal'})
    hist = pd.DataFrame(macd['macd'] - signal['signal']).rename(
            columns = {0:'hist'})
...
def plot_macd(stocks, company, fema, sema, smooth):
    ...
    ax2.plot(macd_df['signal'], color = 'skyblue', linewidth = 1.5, 
             label = 'SIGNAL')
...
# The code was adapted to the program from 
...
def obtain_rsi(stocks, company, win_size, date, price):
    # The changes will be calculated on a daily basis
    change = price.diff(1).dropna() 
    gain = 0 * change
...
def plot_rsi(stocks, company, win_size):
    ...
    price, date = stkols.get_price_date(stocks, company[0])
    month = stkols.get_month(stocks, company[0])
    stocks[company[0]]['rsi'] = obtain_rsi(stocks, company, win_size, date,
          price)
