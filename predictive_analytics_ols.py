...
''' Generate a simple OLS regression model for one data set
    The basis for the OLS model was obtained through the 
    knowledge acquired in the Udemy Course:
...
def sk_simple_OLS(stocks, company, t_size, size):
    ...
    inputs, targets, x_train, x_test, y_train, y_test= gen_model(
            stocks, company, t_size, size)
    reg, predicted_v = model_OLS(inputs, x_train, y_train)
    month = get_month(stocks, company[0])   
    #Generate the Plots and output from the linear regression model 
    #considering if we have 1 or two stocks
    if size == 1:
...
# Generate the SKLearn OLS model to generate the graphs 
def gen_model(stocks, company, t_size, size):
    ...
    inputs2 = sm.add_constant(inputs) 
    x_train, x_test, y_train, y_test = train_test_split(
            inputs2, targets, test_size=t_size, random_state=365)
    return inputs, targets, x_train, x_test, y_train, y_test


# Generate the Basic Statsmodels OLS to generate the graphs 
...
# Plotting the results from the regression model vs. the original scatterplot
...
def plot_OLS2(inputs, targets, predicted_v, reg, company, company2):
    ...
    plt.title('Correlation Between Compan Stocks: ' + company + 
              ' vs. ' + company2,fontsize = 14)
    ...
    for company in companies:
        if company not in list(stocks):
            raise tk.messagebox.showerror("Error", 
                  "Stock not found in downloaded data. Did you use the correct symbol? Please try again")
...
def val_t_size(t_size):
    try:
        ...
        raise tk.messagebox.showerror("Error", 
                              "Please enter a number for your Test Size")
    return t_size


#Test the model to new test data to guarantee the accuracy of the prediction
def sk_test_OLS(x_test, y_test, reg):
    ...
    test_table['Square Error'] = test_table['Residual']*test_table['Residual'] 
    test_table['Difference%'] = np.absolute(
            test_table['Residual']/test_table['Target']*100)
    pd.set_option('display.float_format', lambda x: '%.2f' % x)
    test_table.sort_values(by=['Difference%'])
    return test_table 


#Returns the predicted values and the graphs required for Predictive Analytics
def sk_predval_OLS(stocks, companies, t_size, date, size = 1):
    ...
    str_ols = "The predicted value for " + str(date) + " is: " \
        + str(round(pred_test,2)) + \
...
def OLS_two_stocks(stocks, companies, t_size, size =2):
    RSME, R2, reg = sk_simple_OLS(stocks, companies, t_size, size)
    ols2_str = "The coefficient of correlation between stocks is: " \
        + str(round(R2,2)) +"\nThe RSME for the model is: "+str(round(RSME,2))
    return ols2_str